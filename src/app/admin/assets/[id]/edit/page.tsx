...
const emptyForm: EditForm = { title:"",subtitle:"",description:"",story:"",publishDate:"",archiveDate:"",status:"DRAFT",licenseType:"RIGHTS_MANAGED",publicContactEmail:"",publicContactWebsite:"",seoTitle:"",seoDescription:"",keywords:"",copyrightOwner:"",views:"0",likes:"0",country:"",city:"",location:"" };
...
function Field({label,name,value,onChange,type="text",required=false,min}:{label:string;name:string;value:string;onChange:ChangeHandler;type?:string;required?:boolean;min?:string}){return <label className="grid gap-2 text-sm font-semibold">{label}<input name={name} type={type} min={min} value={value} onChange={onChange} required={required} className="rounded-md border border-border bg-bg px-4 py-2.5 font-normal outline-none focus:ring-2 focus:ring-burnt-orange/40"/></label>}
