...
export default function AdminAssetNewPage() {
  ...
  const pasteFile=(event:React.ClipboardEvent)=>{const image=[...event.clipboardData.files].find(item=>item.type.startsWith("image/"));if(image){event.preventDefault();setFile(image);setError("");}else setError("Clipboard không chứa ảnh. Hãy sao chép trực tiếp hình ảnh rồi dán lại.");};
  ...
  return <div className="max-w-4xl p-6"><div className="mb-8 flex items-center gap-3"><Link href="/admin/assets" className="rounded-md p-2 hover:bg-border-light"><ChevronLeft size={18}/></Link><div><h1 className="text-3xl font-bold">Tải tài sản lên</h1><p className="mt-1 text-sm text-secondary">Tạo hồ sơ đầy đủ cho ảnh hoặc video ngay khi tải lên.</p></div></div>
    ...
    <form onSubmit={submit} className="space-y-8">
      <section onPaste={pasteFile} className="rounded-lg border border-border bg-surface p-6"><h2 className="mb-4 text-lg font-bold">Tệp nội dung</h2><label htmlFor="file" tabIndex={0} className="block cursor-pointer rounded-lg border-2 border-dashed border-border p-8 text-center hover:border-burnt-orange/50 focus:outline-none focus:ring-2 focus:ring-burnt-orange/40"><Upload size={32} className="mx-auto mb-2 text-muted"/><p className="text-sm font-medium">{file?file.name:"Chọn ảnh/video hoặc nhấn Ctrl+V để dán ảnh"}</p><p className="mt-1 text-xs text-muted">Tệp được tải trực tiếp lên Cloudinary · JPEG, PNG, WebP, MP4 hoặc WebM</p>{file&&<span className="mt-3 inline-flex items-center gap-1 text-xs text-olive-green"><CheckCircle2 size={13}/>{(file.size/1024/1024).toFixed(1)} MB</span>}<input id="file" type="file" accept="image/jpeg,image/png,image/webp,video/mp4,video/webm" className="sr-only" onChange={event=>setFile(event.target.files?.[0]??null)} required={!file}/></label></section>
